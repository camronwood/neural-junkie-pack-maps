#!/usr/bin/env python3
"""Pack hub sidecar entrypoint for Maps (stdlib HTTP server)."""
...
class PackHubHandler(BaseHTTPRequestHandler):
    ...
    def do_POST(self) -> None:
        ...
        if path == "/mcp/call":
            ...
            return
        if path.startswith("/api/maps/"):
            self._handle_maps_post(path, body)
            return
        self._json(404, {"error": "not found"})

    def _handle_maps_get(self, path: str) -> None:
        from routes import maps

        maps.handle_get(self, path, self.settings, self.pack_dir)

    def _handle_maps_post(self, path: str, body: dict) -> None:
        from routes import maps

        maps.handle_post(self, path, body, self.settings, self.pack_dir)
